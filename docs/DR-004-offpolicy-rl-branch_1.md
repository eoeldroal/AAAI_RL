...
Status: 분석부 정리 · **미구현** · **이점 미검증**(측정 게이트 대기) · 기본값 `rollout` 유지 시 main 경로 불변
...
- **미구현.** 현재 코드는 `rl_old_logprob_source=rollout`만 허용하며 이 문서의 decoupling(`entry` 모드)은 구현되어 있지 않다.
- 이 DR은 (a) 검증된 현행 동작(§1), (b) 이론 판정(§4), (c) flag-off 구현 스코프(§9)와 (d) 측정 게이트(§11)를 고정하기 위한 기록이다. 코드 변경은 §11의 게이트 통과 후에만 착수한다.
...
3. **정당화 범위.** 그러므로 우월성 주장은 하지 않는다. decoupled는 **문제 적합성**(§6: 이 fork가 구조적으로 RL-branch staleness를 만든다 = decoupled의 정확한 영역)과 **공학 비용**(§9: verl이 flag-off로 제공)으로만 정당화되며, 순이득은 §11 측정으로 게이트된다. CISPO/SAPO/GSPO는 지금 기각하는 대안이 아니라 **결합 가능한 미래 옵션**으로 남긴다.

결합의 엄밀한 판별(무엇이 진짜 결합 가능하고 무엇이 대체/기각인가)과 라우터 출구별 목표 조합은 `DR-005-rl-objective-composition_1.md`가 정식화한다.
...
4. **켜서 이득 여부는 §11 측정 게이트로 판정**한다. 게이트 통과 전에는 ablation을 돌리지 않는다.
...
**권장 결정 — anchor = 옵션 B (recent proximal) 확정.** 편하다는 이유로 옵션 A(기존 재연결)를 고르면 **AReaL decoupled가 아니라 MIS**를 얻는다. 역설적으로 **옵션 B가 A보다 이론 정합적이면서 비용도 낮다**(가중치 스왑이 없음) — A를 고를 유일한 이유는 "코드가 이미 있다"뿐이므로 **기본 채택하지 않는다.** 따라서 phase-1 anchor는 **B로 확정**(§8-2), forward 비용이 실측 병목일 때만 C(A-3PO)를 유보 옵션으로 연다. SFT self-detach와 G5(SFT correction 제외)는 세 옵션 모두에서 loss/헬퍼가 보존한다.
...
`A` vs `B` 한 방은 교란된다 — `A`는 "no correction"이 아니라 "융합(clip이 전체 ρ) + 장치② dormant"이다. 최소 3-arm:

- **A**: 현행 HPT (융합, rollout anchor, correction dormant). HPT-objective 비교 baseline, 그대로 유지.
- **B0**: decoupled(**anchor=옵션 B**, clip on `r`) + 절단 w 적용 + **rejection OFF**. → `B0−A` = **순수 decoupling 효과**.
- **B1**: B0 + rejection ON. → `B1−B0` = **rejection 효과**.
...
## 11. 측정 게이트 (거의 공짜, 구현 전 선행)

전면 구현 전에, 이미 계산되는 두 지표만 상관지어 "이점이 존재할 여지"부터 확인한다.
...
판정(**라우터로 읽을 것** — §4.5):
- **① clip-frac 낮음** → 고칠 게 없음 → 아무것도 구현 안 함.
- **② clip-frac 높고 staleness에 비례** → decoupling의 영역 → §9 착수.
- **③ clip-frac 높으나 staleness와 무관** → 문제는 gradient 死/과클립이지 staleness가 아님 → **decoupled가 아니라 CISPO/Clip-Higher가 정답.**

이 게이트는 전면 구현의 일부 노력으로 §6의 "이점 존재 여부"뿐 아니라 "올바른 도구가 무엇인가"까지 함께 답한다.
...
- **DR-004 (RL branch)**: RL 신호는 rollout provenance가 있어 off-policy 보정이 의미를 가지며, 융합된 단일 ratio를 **decoupling으로 정련할 수 있다**. 단 이점은 조건부라 **측정으로 게이트**한다.
...
§1(현행 동작)·§2~4(장치 정의·crux·이론)는 소스/문헌 검증으로 확정. §6(이점 조건부)는 미측정 사실의 정직한 진술이며 §11이 그 판정. **미구현**이며, §11 게이트 통과 후에만 §9를 착수한다. 파일명 `DR-004`는 DR-003(supervised branch)의 RL-branch 짝이다.
