...
- **단일-요인 격리 — 기준점은 M.** 모든 arm은 M에서 **정확히 한 요소**만 덜어낸다. 공통 기반(DR-001~003 + §2 전제)은 전 격자점 고정, 딸려 흔들리는 축은 통제 손잡이로 고정한다(A1의 β_r, 전 arm의 k_max·γ, M−dec의 `C_g=5.0` 고정). 2×2가 완전하므로 단독 효과(각 코너 vs D0)와 한계 효과(M vs M−X)가 모두 나온다.
...
| `actor.policy_loss.cispo_clip_mode` / `cispo_epsilon_high` | `upper` / `5.0` (`C_g`, absolute ratio cap; lower bound 없음) | C2 |
| 그 외 전부 | §2 공통 기반과 동일 | — |
...
|---|---|---|
| **C1=decoupled** | **M** (앵커, main run) | **M−cispo** (구 A5) |
| **C1=coupled** | **M−dec** (신규) | **D0** (완료 ✅) |
...
**구성.** C1(w-슬롯=TIS-w) + C2(g-슬롯 `min-clip → g(r)=sg(min(r, C_g))`, `C_g=5.0`) 전부 켠 예상 최강점.
...
**필수 통제.** `C_g=5.0`은 PPO `clip_ratio_high=0.28`에서 온 값이 아니라 **CISPO absolute ratio cap**이다. `C_w=2.0`과 곱한 총 계수 상한이 `10.0`이 되어 기존 `clip_ratio_c=10.0`의 안전 스케일과 맞는다. lower bound는 두지 않는다. min()의 비관성(악화 방향에서만 gradient) 상실을 감시.
...
**통제.** `C_g=5.0`은 M과 **동일 값 고정**(캡 의미는 달라져도 값 고정이 단일-요인 조건). validation은 coupled+cispo를 RAISE가 아니라 **WARN**으로 허용한다(이 arm의 존재가 이유).
