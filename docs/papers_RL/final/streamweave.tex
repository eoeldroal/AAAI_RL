...
HybridFlow (EuroSys 2025) separates computation and data dependencies in RLHF, providing an execution substrate for flexible algorithms and resource mappings.
Asynchronous RLHF (ICLR 2025) directly decouples generation from learning and studies the trade-off between execution efficiency and policy lag when training on samples from earlier policies.
AReaL (NeurIPS 2025) fully disaggregates rollout and training workers and combines workload balancing with staleness-aware optimization in an end-to-end asynchronous LLM RL system.
TBA (NeurIPS 2025) trains an off-policy objective on replay experience collected by asynchronous actors.
Laminar (EuroSys 2026) independently executes trajectories in GRPO to absorb variation in generation latency within a group.
...
In general reinforcement learning, DQfD (AAAI 2018) uses demonstrations in temporal-difference updates and a supervised loss, while RLPD (ICML 2023) continually incorporates offline data into online RL.
In LLM post-training, InstructGPT (NeurIPS 2022) stages demonstration-based supervised learning before RLHF, and SimpleMix (ICML 2025) directly mixes on-policy and off-policy data for preference learning.
For reasoning post-training, LUFFY (NeurIPS 2025) combines off-policy reasoning traces with policy rollouts through mixed-policy learning.
CHORD (ICLR 2026) reformulates SFT as a dynamically weighted auxiliary objective optimized alongside on-policy exploration.
ReLIFT and SRFT also use expert trajectories for reasoning post-training.
...
Our experiments instantiate the source rule with Hybrid Post-Training (HPT), which selects between a policy group and an expert trajectory from the outcome of a complete rollout group.
...
For the policy source, StreamWeave retains all $n$ rollouts and compares their verifier scores to compute the GRPO advantage $A^{\mathrm{grp}}_{r,t}$.
...
Our experiments instantiate $\Phi$ with vanilla clipped PPO.
...
Oat-Zero & 17.2 & 12.6 & 49.6 & 73.7 & 30.1 & 38.1 & \textbf{36.9} \\
LUFFY & 15.1 & 14.0 & 46.0 & 77.5 & 30.0 & 43.5 & \textbf{37.7} \\
\textbf{StreamWeave} & \textbf{16.1} & 13.0 & 47.0 & \textbf{78.5} & \textbf{33.0} & 43.2 & \textbf{38.5} \\
\bottomrule
\end{tabular}
...
StreamWeave starts from Qwen2.5-Math-1.5B and trains on prompts paired with verified expert trajectories from OpenR1-Math.
...
We perform fixed-checkpoint evaluation on six mathematical reasoning benchmarks: AIME24, AIME25, AMC (83 problems), MATH500, Minerva (272 problems), and Olympiad (674 problems).
...
Training reward and our mathematical evaluations use the same Math-Verify grader.
We separately evaluate the same checkpoints on ARC-Challenge, GPQA-Diamond, and MMLU-Pro, reporting their macro-average as cross-domain reasoning performance.
Baselines include the base and instruct models, Async RL (expert-off), and methods that use RL or expert trajectories.
