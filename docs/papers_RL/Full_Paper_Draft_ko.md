...
source를 넓힌다. 이 논문이 다루는 group-conditioned setting에서는 complete group의 결과가 policy와
expert 중 어느 data와 update를 사용할지 결정한다. 이를 trajectory-level full asynchrony와 결합하면
runtime의 도착 순서와 batching이 그 결정을 다시 쓸 수 있다. StreamWeave의 중심 통찰은 이 판단에
필요한 boundary가 global synchronization barrier로 되돌아올 필요가 없다는 것이다. StreamWeave는
필요한 맥락을 국소적으로 복원하고 source별 학습 역할을 유지하면서도 fully-asynchronous pipeline을
계속 전진시킨다.

**Novelty kernel과 claim boundary:** novelty는 비동기 실행, expert trajectory, source selector 중
어느 부품에도 단독으로 있지 않다. Complete group이 source와 update를 함께 결정하는 환경에서는
group completion이 단순한 data-availability condition을 넘어 control dependency가 된다. StreamWeave가
소유하는 지점은 이 dependency를 global barrier로 확장하지 않고 full asynchrony 안에서 실현하는
composition architecture다.
...
Learner contract는 독립 기여나 그 자체로 올바름을 증명하는 정리가 아니라, 기여 1의 learning
composition을 압축하는 명세다. 실제 objective가 이 명세를 만족함은 §3.1의 구성적 유도로, 그
composition이 full asynchrony 아래에서 실현됨은 §3.2의 architecture로 회수한다. Unit test와
contract test는 코드 저장소의 implementation QA이며 논문의 evidence hierarchy에 넣지 않는다.
공개 핵심 약속은
...
**증거 위계:** headline은 quality SOTA가 아니라 같은 learning composition 아래의 실행 효율이다.
Matched synchronous control의 quality 보존이 architecture 비교의 전제를 닫고, `1.64x` throughput이
end-to-end payoff를, `54.7% -> 3.2%` stall이 그 기전을 설명한다. Strongest hybrid baseline 대비
`+0.8` points는 경쟁력을 보이는 보조 증거이며, 통계적 근거 없이 보편적 우월성으로 확대하지 않는다.
...
| 위치 | 남길 내용 |
|---|---|
| **Abstract** | 관찰 가능한 compute–signal bottleneck, group-conditioned setting의 범위, 실행 충돌과 학습 차이, `trajectory-level execution → local group reconstruction → source-appropriate update`, 비교 방법 전반과 matched synchronous quality, same-hardware 작업 시간과 trainer 대기 비율 |
...
Abstract의 execution-first 설명은 독자의 이해를 위한 서사 순서이며 Method의 증명·설계 위계를
뒤집지 않는다. 공개 Abstract에는 accumulator, queue, mask, self-detach를 명명하지 않고 그 기능만
설명한다. 결과는 품질 한 문장과 efficiency 한 문장으로 닫으며 별도의 반복 결론을 덧붙이지 않는다.
...
> StreamWeave는 먼저 complete group이 source-specific update로 변환되는 learning composition을
...
여기서 learning definition은 모든 heterogeneous learning에 대한 보편 공리가 아니라, 이 논문이
다루는 **group-conditioned policy/expert composition의 정체성 조건**이다. 구성적 유도는 추상적인
contract를 다시 증명하는 것이 아니라, 실제 StreamWeave objective를 대입해 각 branch가 선언한
contribution으로 환원됨을 보인다. StreamWeave가 소유하는 주장은 임의의 hybrid algorithm에 대한 유일
해법이 아니라, 이 composition을 full asynchrony 아래에서 변형하지 않고 실현하는 원리와 architecture다.

전체 논증은 다음 순서를 따른다.

```text
Learning composition
  어떤 mixed update를 유지할 것인가
        ↓
Constructive derivation
  실제 objective가 그 composition으로 환원되는가
        ↓
StreamWeave execution design
...
Empirical evaluation
  학습 효과와 실행 효율이 함께 나타나는가
```

Contract는 첫 수준을 압축하고, 구성적 유도는 실제 objective가 그 정의를 만족함을 보인다. 이어지는
architecture는 수학적으로 유일한 해법으로 주장하지 않지만, 정의된 composition과 비동기 효율을 함께
달성하기 위해 StreamWeave가 선택하고 소유하는 핵심 기여다. 구현 테스트는 이 논증 사슬 밖에서 코드
품질을 관리하며, 논문에서는 수학적 유도나 과학적 실험을 대신하지 않는다.

§4가 composition requirement, core design, inherited substrate의 지위를 소유한다. 이 절에서는 중복을
피하고 현행 시스템이 선택한 concrete realization만 기록한다.
...
않는다**는 것이다. 이 절은 장치 목록이 아니라, 각 composition requirement를 효율적으로 실현하는
이유를 보여주는 논증으로 쓴다. 핵심 문단은 `requirement의 요구 → 단순 구현의 효율 손실 또는 의미 왜곡 → StreamWeave의
설계 → 의미와 효율을 함께 유지하는 결과`의 인과를 갖는다.

§3.2의 세 논증 단위는 §4 authoritative design ledger의 순서와 명칭을 그대로 따른다. 각 문단은 해당
boundary의 `requirement → naive failure → StreamWeave design → 함께 유지되는 성질`만 풀어 쓰고,
별도의 충돌 분류나 계약 명칭을 만들지 않는다.

Independent attempt scheduling, bounded queue, backpressure, parameter refresh는 fully-asynchronous
runtime에서 상속한 실행 기반이다. 이들은 StreamWeave가 end-to-end로 작동하는 데 필요하지만 별도의
설계 기여로 나열하지 않고, §3.2 말미에서 기존 flow control을 유지한다는 한 문장으로만 회수한다.

전체 서술은 `inherited asynchronous freedom → added group dependency → local reconstruction →
source-before-transport → context-preserving handoff → end-to-end overlap` 순서로 진행한다. AReaL의
