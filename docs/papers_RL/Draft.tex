% PAPER STATUS (2026-07-22): legacy implementation/Appendix source.
% The public method name, thesis, claims, and main-run evidence are owned by
% Full_Paper_Draft_ko.md. "Async-HPT" and the computer-use framing below record
% an earlier implementation narrative and must not be copied into the main paper verbatim.
...
RL row의 reference에는 한 가지 변형이 있다. 비동기 실행에서는 $\ell^{\mathrm{rollout}}_{r,t}$ 대신 학습 시점에 재계산한 proximal old-policy의 log-probability를 reference로 둘 수 있다(decoupled anchor). 이때 ratio의 clip은 순수하게 이번 update의 이동만을 제약하고, 생성 정책과의 잔여 mismatch는 A.5의 곱셈 보정이 별도 슬롯에서 흡수한다. 본문의 주 실험은 이 decoupled 구성으로 실행되었으며, 실측에서 두 reference의 차이는 근소했다(곱셈 보정의 절단 포화율 $\sim$0.1\%) — 즉 이 변형은 보수적 보험이지 결과를 가르는 축이 아니다.
