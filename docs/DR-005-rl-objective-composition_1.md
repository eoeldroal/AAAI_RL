...
- **C2(upper-only CISPO config+loss) 구현·테스트 완료**(2026-07-04). 이 문서는 그 위에서 "무엇을 결합해도 수학적으로 성립하는가"의 판별 기록이며, 결합 효과 자체는 사후 귀속 대기다.
- 적용 순서는 `Ablation_RL.md`의 M-first 2×2가 정한다. 이 문서의 역할은 M의 두 슬롯(C1=w, C2=g)이 수학적으로 결합 가능한지와, 사후 귀속에서 어떤 지표를 읽어야 하는지를 고정하는 것이다.
- "최적"의 범위 한정: 여기서 최적은 §2의 제약(신선 극한 불편성, 계수 유계, 계약 보존, per-token 단위) **아래에서의** 조건부 최적이며, 전역 최적성 정리가 아니다. M-first에서는 g-슬롯을 upper-only CISPO로 먼저 채택하고, 효과와 안정성은 사후 지표가 판정한다.

---

## 0. 한 문단 요약

CISPO·SAPO·GSPO·Clip-Higher·decoupled+TIS는 서로 다른 알고리즘이 아니라, **하나의 per-token 추정량 족(family)의 두 슬롯에 대한 선택지**로 정리된다: `∇Ĵ = E[ w̄_t · g(r_t) · A_t · ∇log π_θ ]`. 여기서 **w-슬롯**(`w̄_t`, staleness 보정)은 θ-무관 상수 계수이고 **g-슬롯**(`g(r_t)`, trust-region gate)은 θ-의존 함수라, 두 슬롯 사이에 gradient 교차항이 없다(Lemma 1). 이 **슬롯 분리(disjointness)**가 "진짜 결합 가능"의 정확한 판별 기준이다 — 서로 다른 슬롯을 차지하면 결합 가능(decoupled×{hard-clip, Clip-Higher, dual-clip, CISPO, SAPO}), 같은 슬롯을 차지하면 결합이 아니라 대체(CISPO×SAPO), 슬롯의 단위 구조를 바꾸면 결합 불가(GSPO — token→sequence 단위 변경은 DR-001 aggregation 계약·Dr.GRPO 인센티브 제약과 충돌). w-슬롯에는 정준(canonical) 선택이 존재한다: `w̄ = min(w, C)`(per-token TIS)는 상한 C 아래에서 pointwise 편향 최소(Lemma 4)이며 verl이 이미 제공한다. g-슬롯에는 정준 선택이 없고 레짐이 정한다 — decoupling이 CISPO·SAPO의 gradient-死 레짐을 축소하는 정도는 **anchor 종류에 달렸다**(§1.1 주의). M-first 설계는 **decoupled + TIS-capped w(C_w=2.0) + upper-only CISPO(C_g=1.28=1+ε_h)** 를 먼저 실행하고, `M−cispo`/`M−dec`/D0로 사후 귀속한다. 모든 채택 조합은 `g(1)=1`을 만족하므로 SFT branch는 **구성상(by construction)** 불변이다(Lemma 3).
...
| CISPO | (원논문은 roll=entry라 슬롯 미분리) | `sg(min(r, C_g))`, upper-only | **절대 0이 안 됨** (계수 포화) |
| SAPO | (없음 — off-policy 보정 자체가 없음) | `4σ'(τ_±(r−1))·r` | 매끄럽게 감쇠, 0에 점근하나 도달 안 함 |
| GSPO | — | — | **per-token 슬롯 구조 이탈** (sequence 단위 ratio) |

세 gate 모두 `g(1)=1`을 만족한다(hard clip: r=1은 밴드 안; CISPO: `min(1, C_g)=1` when `C_g>=1`; SAPO: 4σ'(0)·1=1). 이 성질이 §2의 계약 보존을 만든다.
...
**열린 결정 (M−cispo/D0 baseline g-slot):** baseline g-slot을 (A) 런처 그대로 **Clip-Higher 유지**할지, (B) UPT 파리티/최소-clip 원칙에 맞춰 **대칭 0.2/0.2로 낮출지**는 아직 미정이다(M의 g-slot은 별개로 upper-only CISPO 확정) — UPT **성능 비교**가 목적이면 (B)(또는 clip-higher를 async-forced 레시피로 명시), **decoupling ablation**만이 목적이면 (A)로 충분. **아래 §4.2~§6의 "vanilla min-clip" 표현은 (B) 방향의 이상형이며, 현행 코드는 (A)임을 유의.** (DR-004 §1-8도 이 실태를 기록.)
...
따라서 "decoupling이 CISPO 레짐을 없앤다"는 **옵션 B에서만 강하게 성립**한다. 옵션 A(현행 재연결)에서는 축소가 제한적이다. 어느 쪽이든 예측이지 측정이 아니므로(§8), M-first run 뒤 `M−cispo`/`M−dec`/D0 델타가 실제 기여를 판정한다 — 그리고 **어느 anchor를 쓰는지가 이 예측을 바꾸므로 §6에서 anchor를 명시**한다.

### 4.2 후보별 정밀 비교 (모두 Lemma 1–3 통과 — 결합은 전부 성립, 문제는 선택)

**(a) vanilla min-clip (+dual-clip) — M−cispo/D0의 baseline g-slot.**
...
**(b) Clip-Higher(ε_h>ε_l) — baseline에 대한 저비용 옵션 (현행 M−cispo/D0가 이미 사용).** min() 비관성을 유지한 채 상방 여유만 확대(entropy 붕괴 완화). 파라미터 2개짜리 변경이라 entropy 추이를 보고 결정.

**(c) CISPO-sg — M의 g-슬롯 선택.**
```
L_t = − w̄_t · sg( min(r_t, C_g) ) · A_t · log π_θ
```
- cap 값: `C_g = 1 + clip_ratio_high = 1.28`로 둔다. **근거는 partition 스케일 매칭이다.** decoupled M에서 g-슬롯의 `r = π_current/π_entry`는 **이동(movement) partition만** 담당하고(staleness는 w-슬롯 `C_w=2.0`이 분리 흡수), 이동 partition은 MiniMax-M1의 on-policy IS ratio(roll=entry, 배치 내 이동)와 같은 종류·같은 스케일이다. 우리는 `ppo_epochs=1`·fit_step당 SGD≈4라 원논문(16회 갱신)보다 이동이 오히려 작다 → 캡은 이동 스케일(≈1.2~1.3)이어야 한다. M−cispo(vanilla Clip-Higher)의 상단 밴드(`ε_h=0.28`)와 값을 맞춰 `1.28`로 두면 C2 축이 "같은 지점에서 gradient 死(vanilla) vs 유지(CISPO)"만으로 갈린다(밴드 정렬). **주의: ScaleRL/ms-swift가 쓰는 `C_g=5.0`은 coupled·완전 async ratio(staleness 포함)용 값이라 이동 partition 캡에 이식하면 스케일 오배정이며, 그 자리는 M−dec다** — "전체 계수 상한 10.0" 프레이밍(=`C_w·C_g`)은 coupled/decoupled 레짐을 뒤섞은 것이라 폐기한다. 구현은 upstream verl 단일 채널(`clip_ratio_low`/`high`)로 하고 하한은 `clip_ratio_low=10`으로 비활성(원논문 lower bound 비활성 관례). MiniMax 원논문은 ε^IS_high 수치를 미공개(tuned)하며, verl `examples/cispo_trainer`는 이를 `clip_ratio_high=0.2`(cap 1.2)로 구현한다.
- 결합의 이점: 원논문은 roll=entry(신선 batch 다회 갱신)라 caps가 하나뿐이지만, decoupled 합성에서는 **staleness 캡(C_w)과 이동 캡(C_g)이 분리**되어 각자의 의미가 깨끗해진다 — coupled 상태에서 CISPO화하면 두 캡이 하나의 ρ 위에서 충돌한다. 즉 CISPO는 decoupling과 결합할 때 오히려 원형보다 정합적이다.
...
**(d) SAPO gate — 후순위 후보 (불연속/고분산 실측 시; §6 원장 제외).**
```
...
| 조합 | 판정 | 근거 |
|---|---|---|
| decoupled(TIS-w) × vanilla min-clip(+dual-clip) | ✅ **성립 (M−cispo)** | Lemma 1–3 통과, Lemma 2로 verl 적용점 그대로 정확 |
| decoupled × Clip-Higher | ✅ 성립 (M−cispo/D0의 baseline g-slot) | 밴드 파라미터만 변경, 비관성 유지 |
| decoupled × CISPO-sg | ✅ 성립 (M의 C2) | 슬롯 상이 + g(1)=1; 캡 2개의 의미 분리는 원형보다 정합적. 비관성 상실은 수용 비용 |
| decoupled × SAPO gate | ✅ 성립 (후순위 후보) | 슬롯 상이 + g(1)=1; τ 재튜닝 필수 |
...
**M / M−cispo 공통 C1(decoupling) 구조:**

```
RL token:  M−cispo: − w̄_t · [ vanilla min-clip(r_t, A_t; ε_l, ε_h) with dual-clip(c) ]
           M:        − w̄_t · sg(min(r_t, C_g)) · A_t · log π_θ, C_g=1+ε_h=1.28
...
rejection: OFF · k_max: 유지 · M의 g-slot: **upper-only CISPO(C_g=1.28=1+ε_h; clip_ratio_low=10으로 하한 비활성)**. M−cispo/D0의 g-slot은 현행 launcher의 Clip-Higher(0.2/0.28)+dual-clip(10)(§4.0)
```

이것은 HCS 2022의 decoupled objective + AReaL의 TIS를 verl의 기존 적용점(`rollout_is_weights`)에 실현한 것이다. **정정(초안 과장 교정)**: w-슬롯(`rollout_is_weights` 곱셈)은 기존 부품의 재배선이 맞지만, **`π_entry`(=old_log_probs)를 어떤 anchor로 계산하느냐는 신규 결정**이다(DR-004 §9 옵션 A/B/C). "기존 `_compute_old_log_prob` 재연결"은 옵션 A(MIS θ_sync)를 주며 **AReaL식 recent proximal이 아니다.** 진짜 AReaL식(옵션 B)은 old_log_probs를 매 fit_step 현재 가중치로 forward하는 것으로, 가중치 스왑이 없어 오히려 MIS보다 싸다. 즉 C1을 "그대로 재배선"으로 표현하면 안 되고, **anchor 선택이 C1의 핵심 결정**이다.

**M-first 이후 사후 귀속:**

- **M−cispo 대비 M 개선** + r-clip 死가 피벗 토큰에 집중 → CISPO-sg 채택 근거.
- **M−dec 대비 M 개선** + `P(w>C_w)`/staleness 서명 → decoupling + TIS-w 채택 근거.
- **둘 다 미약** → 이 레짐에서는 full stack이 조건 미충족. SAPO/A-3PO/rejection은 별도 후순위 후보로만 남긴다.

**M−dec (coupled+CISPO — staleness 무관·movement 死 실측용 arm)**: decoupling 없이 g-슬롯만 CISPO로 켠 코너. 이 경우에도 본 문서의 판별은 유효하다 — coupled 상태의 CISPO는 캡 충돌(§4.2c)이 있으므로 **M은 decoupling을 함께 켜 두 캡을 분리**하고, M−dec는 그 충돌을 실측한다. **캡 값은 M과 동일하게 `C_g=1.28`로 고정**한다(단일-요인 조건: M↔M−dec는 C1만 달라야 함). 이 이동용 캡이 coupled ρ(staleness 포함)에 걸리면 대부분 토큰이 상단 포화 → 이동 캡이 staleness까지 떠안는 충돌이 그대로 관측된다. (ScaleRL/ms-swift가 coupled·async에 쓰는 `C_g=5.0`은 "잘 튜닝된 coupled CISPO"용이라 이 arm의 목적(충돌 실측)과 다르고 단일-요인 격리도 깨므로 채택하지 않는다.)
...
2. **캡 충돌**: coupled 상태에서 CISPO화하면 staleness 캡과 이동 캡이 하나의 ρ 위에서 충돌한다. decoupling이 이 충돌을 해소한다(§4.2c). M−dec는 이 충돌을 실측하기 위한 arm이지 M의 설계가 아니다.
3. **하이퍼 이식 금지 — 캡은 자기 partition 스케일에 맞춘다**: gate 인자가 ρ→r로 바뀌는 순간 ε·τ·임계값의 의미가 바뀐다. M의 g-슬롯 `r`은 **이동 partition**이라 캡도 이동 스케일이어야 한다 — `C_g=1+clip_ratio_high=1.28`(M−cispo 밴드 정렬). coupled ρ(staleness 포함)용 값(ScaleRL `5.0`)을 이동 캡에 이식하면 스케일 오배정이다. `C_g`는 upper-only ratio cap이고 하한은 `clip_ratio_low=10`으로 비활성(원논문 관례). ※ 이동 캡이 M−cispo의 상단 밴드(0.28)와 값이 같은 것은 "PPO ε 이식"이 아니라 C2 축 격리를 위한 **의도적 밴드 정렬**이다.
...
| r-clip-frac (decoupling 후) | §4.1 구조 예상의 검증; M−cispo 대비 M의 정보성 |
| death 밀도: clip된 토큰 중 low-prob(피벗형) 비율, A 부호별 | M에서 CISPO가 실제로 한 일을 판정 (staleness와 무관한 死) |
...
| DR-004 (decoupling 결정, flag-off, §11 사후 귀속) | **본 문서가 §4.5 격자의 정식화.** M-first 2×2에 검증된 조합을 배치(§6). 귀속(HCS 2022) 일치 |

---

## 부록: 상태와 명명

§1–2(통일족·판별 기준)와 §5(판정표)는 수학적 사실 + 문헌·코드 검증 위에 있다. §4.1(레짐 축소)과 §6(목표 조합)은 구조 논증이며 최종 판정은 §8 계측이 한다. **C2는 구현 완료**이고 M의 g-슬롯으로 채택됐으며, 결합 효과의 판정은 M/M−cispo/M−dec/D0 사후 귀속(§8)이 한다. 파일명 `DR-005`는 DR-004(decoupling 단일 결정)의 상위 결합 문법이다.
...
**정정 3 (M-first / CISPO cap)**: `Ablation_RL.md`가 M-first 2×2로 재배향되면서, 이 문서의 "DR-004 §11 라우터 통과 후 CISPO 채택" 표현은 사후 귀속으로 격하됐다. M의 g-slot은 **upper-only CISPO**이고 lower bound는 두지 않는다(`clip_ratio_low=10`).

**정정 4 (CISPO cap 스케일·구현 채널 정정, 2026-07-04)**: 초안은 `C_g=5.0`(ScaleRL/ms-swift 값)을 M의 이동 캡으로 쓰고 "총 계수 상한 10.0"으로 정당화했으나, 이는 **coupled·off-policy ratio용 값을 decoupled의 이동 partition 캡에 오배정**한 것이었다(§4.2c 스스로 선언한 "이동 캡/staleness 캡 분리"와 자기모순). 정정: **M·M−dec의 `C_g=1.28`=`1+clip_ratio_high`** (이동 스케일; M−cispo 밴드 정렬로 C2 축 격리), staleness는 w-슬롯(`C_w=2.0`)이 담당. `5.0`은 어느 격자점에도 쓰지 않는다. 구현 채널도 정정: 포크가 덧댄 `policy_loss.cispo_clip_mode`/`cispo_epsilon_high`(절대-cap 이중 채널)를 **제거**하고 upstream verl 단일 채널(`clip_ratio_low`/`high`)로 복원 — `examples/cispo_trainer` 회귀도 함께 해소(§7-3).
