...
```
- cap 값: 첫 M run은 `C_g=5.0`으로 고정한다. 이는 PPO `clip_ratio_high=0.28`의 이식이 아니라 **CISPO absolute ratio cap**이다. `C_w=2.0`과 결합해 전체 계수 상한을 `10.0`으로 맞춘다. MiniMax-M1/SWIFT 계열의 upper-only CISPO 관례를 따르되, 이 값은 "논문 정답값"이 아니라 문헌 방향 + 현재 런처의 안전 스케일을 맞춘 첫 main 값이다. lower bound는 두지 않는다.
...
RL token:  M−cispo: − w̄_t · [ vanilla min-clip(r_t, A_t; ε_l, ε_h) with dual-clip(c) ]
           M:        − w̄_t · sg(min(r_t, C_g)) · A_t · log π_θ, C_g=5.0
...
rejection: OFF · k_max: 유지 · M의 g-slot: **upper-only CISPO(C_g=5.0)**. M−cispo/D0의 g-slot은 현행 launcher의 Clip-Higher(0.2/0.28)+dual-clip(10)(§4.0)
```
...
**M−dec (coupled+CISPO — staleness 무관·movement 死 실측용 arm)**: decoupling 없이 g-슬롯만 CISPO로 켠 코너. 이 경우에도 본 문서의 판별은 유효하다 — coupled 상태의 CISPO는 캡 충돌(§4.2c)이 있으므로 **M은 decoupling을 함께 켜 두 캡을 분리**하고, M−dec는 그 충돌을 실측한다.
...
3. **하이퍼 이식 금지**: gate 인자가 ρ→r로 바뀌는 순간 ε·τ·임계값의 의미가 전부 바뀐다. PPO `clip_ratio_high=0.28`을 CISPO `C_g`로 이식하지 않는다. M의 `C_g=5.0`은 absolute ratio cap이다.
...
**정정 3 (M-first / CISPO cap)**: `Ablation_RL.md`가 M-first 2×2로 재배향되면서, 이 문서의 "DR-004 §11 라우터 통과 후 CISPO 채택" 표현은 사후 귀속으로 격하됐다. M의 g-slot은 **upper-only CISPO**이고 첫 값은 `C_g=5.0`이다. `C_g`는 PPO clip의 `1+ε`가 아니라 absolute ratio cap이며, lower bound는 두지 않는다.
