...
- 완료: HPT learner batch 계약을 `hpt_is_sft` 중심으로 축소.
- 완료: `B_eff`, `hpt_seq_weight`, `hpt_length_divisor`, `hpt_loss_denominator`, `finalize_loss_denominator`, HPT 전용 PPO surrogate 재구현 제거.
- 완료: SFT row는 `old_log_prob = log_prob.detach()`로 self-detach한 뒤 표준 `get_policy_loss_fn("vanilla")` 경로를 사용.
- 완료: `async_hpt.loss_aggregation=branch_blind`, `sft_beta_mode={constant,length_inverse}`, `beta`, `alpha==1.0` 검증, `loss_scale_factor` 기반 길이-반비례 beta 검증 추가.
- 검증: `/home/sogang_nlpy/miniconda3/envs/RL/bin/python -m pytest tests/special_RL -q` 통과.
- 미구현: branch별 gradient-mass 계측, `hpt/tau_action_ratio`, DR-003 belt/entry snapshot.
...
- [x] `losses.py`에서 HPT 분기를 표준 vanilla 경로 + `loss_agg_mode` 배선으로 교체.
- [x] assembler에서 B_eff/weight/divisor 필드 생성 제거. SFT pseudo reward를 `β_r` 인터페이스로 노출(상수 기본값, 길이-반비례 옵션).
- [x] `loss_scale_factor = L_max` 설정 경로 확인 및 config 노출.
- [x] 등가성 테스트: all-RL HPT batch가 vanilla PPO loss와 스케일 일치, SFT self-detach ratio=1, `β_r = β·L_max/|o|` 설정 검증.
- [ ] 관측 지표 추가: round별 branch별 gradient-mass 점유율, `hpt/tau_action_ratio`(§5 측정).
- [x] 미해결(별건, DR 분리 예정): KL/entropy가 SFT row에 균일 적용되는 문제는 DR-002로 분리하고 RL-only auxiliary mask를 구현.
...
이 기록은 설계 결정을 담은 것이며, branch-blind loss refactor의 CPU 계약 테스트는 통과했다. GPU/장기 E2E run은 별도 승인 후 최종 확인 대상으로 남긴다. 파일명 `DR-001`은 후속 결정 기록(τ* 무결성, KL/entropy 처리, 실패-원자성 등)을 위한 번호 체계의 시작이다.
