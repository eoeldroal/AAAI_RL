...
Status: 분석부 확정 · 안정성 가설 미검증(사전 계측 후 ablation 대상) · 현재 구현 범위는 self-detach 유지
...
관련 코드: `verl/workers/utils/losses.py` (effective old log-prob self-detach 126행), `verl/trainer/ppo/rollout_corr_helper.py` (SFT correction 면제 1061행)
...
τ*는 우리 정책의 rollout이 아니라 고정 target(정답 혹은 teacher 산출물)이다. "이 데이터를 만든 우리 정책"이 없으므로 빈도 교정의 대상이 아니다. 따라서 **보정 가중치 = 1**(곱해도 무변). 이것이 A.5의 "SFT는 IS correction 면제"의 근거이며, 특혜가 아니라 대상의 부재다. 코드에 이미 반영(rollout_corr_helper:1061). — **확정, 구현됨.**
