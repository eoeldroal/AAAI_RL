...
# ============================================================================
# [RLonly v2 = 순수 async GRPO, 교사 채널 봉인] Ablation_RL.md §13 H-축 arm (2026-07-10)
# 목적: 하이브리드 교사 채널(미해결→LUFFY SFT)의 순기여 격리. nocispo(main) − RLonly = H.
#
# ★v1 사후분석(2026-07-10 06:41 런, 크래시)★: async_hpt.enabled=False로 끄자
#   비-HPT 경로가 old_log_prob 재계산 시 version-1 save/restore(MIS 장치,
#   fully_async_trainer._compute_old_log_prob)로 진입 → actor_save_model_to_cpu가
#   "No DTensor-type parameters found"(fsdp2 비호환)로 즉사. HPT 런들은 entry 숏컷/rollout
#   앵커로 이 경로를 우회해 와서 미노출 잠복 버그였음.
#
# v2 메커니즘 — HPT는 켜둔 채 라우팅만 봉인 (단 한 줄):
#   async_hpt.success_threshold: 0.0 -> -1.0
#   → count_successful_rollouts: score(0/1) > -1.0 항상 참 → success_count≡8 → p_success≡1
#   → route_to_sft = (1 ≤ γ=0) = False → SFT 라우팅 절대 불발, k=0 그룹은 adv 0의 RL 행으로
#     잔류(순정 GRPO 의미론 = 처치 그 자체). tau 로드는 되나 사용 안 됨(p>γ라 missing_tau 무해).
# v1 대비 장점: entry 앵커·token-IS·큐·스케줄러가 main과 100% 동일 → 축이 정말 하나만 움직임
#   + 크래시 경로 원천 회피.
# 예상 메트릭 서명(정합성 체크용): hpt/num_sft≡0, hpt/onpolicy_success_rate≡1.0(집계 왜곡,
#   무해 — 진짜 성공률은 critic/score/mean으로 판독), offline_data_ratio≡0.
# 판독: matched-step 50-190 창 vs nocispo. 부수: 순수 GRPO의 폭풍 유무.
# ============================================================================
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_RLonly_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_RLonly_${RUN_TIMESTAMP}"
...
VAL_DUMP_DIR="${VERL_ROOT}/.cache/val_dump/openr1_async_hpt_qwen25_math_1_5b_RLonly_${RUN_TIMESTAMP}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_RLonly_grpo \
    ...
    async_hpt.gamma=0.0 \
    async_hpt.success_threshold=-1.0 \
    async_hpt.beta=0.3 \
