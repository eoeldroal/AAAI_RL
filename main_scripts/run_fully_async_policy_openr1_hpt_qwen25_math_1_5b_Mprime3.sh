...
# ============================================================================
# [Mprime3] Mprime2(length_inverse scratch) 위에 entropy만 켜는 독립 arm.
# Mprime2 대비 학습 동작 델타는 정확히 하나다:
#   1) actor_rollout_ref.actor.entropy_coeff=0.001 — clean RL row에만 entropy
#      bonus를 부여해 탐색 여지를 복원한다. SFT row는 기존처럼 제외되고,
#      P0-2 dead row인 truncated RL row도 entropy mask에서 제외된다.
#
# Mprime2의 scratch/no-resume 기조, length_inverse, beta=0.6, P0-1/P0-2,
# SGLang/runtime/async budget은 그대로 유지한다. experiment/dump 이름만
# Mprime3로 분리한다.
# ============================================================================
...
# [각주 0 · Mprime3 = scratch arm] 이 스크립트는 M run(_M.sh) 공통 기반 위에 docs/Improvement_RL.md
# §5의 P0 조치, Mprime2 길이 개입(length_inverse + beta=0.6), 그리고 entropy 복원을
# 얹은 **독립 scratch run**이다.
# M.sh/Mprime.sh는 기존 앵커로 보존하고, Mprime3는 entropy arm으로 실험명만 분리해 새 arm으로 기록한다:
#   - P0-1: +reward.reward_kwargs.zero_reward_if_truncated=True (truncated ⇒ reward 0; 라우팅·baseline·보상 동시 교정)
#   - P0-2: +reward.reward_kwargs.zero_truncated_rl_advantage=True (advantage 계산 후 truncated RL row의 advantage 0)
#   - P0-3: actor.calculate_entropy=True (진단지표 유지)
#   - Mprime3-1: async_hpt.sft_beta_mode=length_inverse
#   - Mprime3-2: async_hpt.beta=0.6
#   - Mprime3-3: actor.entropy_coeff=0.001 (truncated RL row 제외 후 entropy 복원)
# resume 금지: 이어받기 런은 같은 이름의 clean ablation이 아니라 별도 continuation이므로 여기선 쓰지 않는다.
#
# [각주 1] 이 스크립트는 Qwen2.5-Math-1.5B용 async RL + HPT의 **M(예상 최강) run**이다.
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_Mprime3_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_Mprime3_${RUN_TIMESTAMP}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    +reward.reward_kwargs.zero_reward_if_truncated=True \
    +reward.reward_kwargs.zero_truncated_rl_advantage=True \
    "trainer.logger=['console','wandb']" \
    trainer.project_name=async-hpt-openr1 \
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_hpt_Mprime3_entropy001_v1 \
    ...
    async_hpt.sft_beta_mode=length_inverse \
