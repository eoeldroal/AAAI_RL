...
# [M2] M launcher의 공통 기반을 유지한 scratch arm이다.
# M 대비 의도된 학습 로직 델타는 둘이다:
#   - async_hpt.sft_beta_mode=length_inverse
#   - async_hpt.beta=1.0
# 즉 SFT terminal pseudo-reward를 beta·8192/len(tau)로 바꾸고 총량도 M(beta=0.3) 대비
# 키워, 현재 fork의 길이 편향 제거와 SFT 채널 강화 효과를 함께 본다.
# 그 외 M의 공통 기반(C1 decoupling + C2 CISPO, async stack, batch budget)은 유지한다.
# clean 비교를 위해 resume는 금지하고, step-0 val anchor를 복구한다.
#
# [각주 1] 이 스크립트는 Qwen2.5-Math-1.5B용 async RL + HPT의 **M2(score-oriented length-equalized) run**이다.
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_M2_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_M2_${RUN_TIMESTAMP}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_hpt_M2_sft_leninv_beta1 \
