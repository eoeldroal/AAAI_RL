...
# ============================================================================
# [D0prime = coupled + vanilla] Ablation_RL.md §13 both-off 코너 (현세대) 2026-07-10
# 새 main=nocispo(decoupled+vanilla) 기준의 C1(decoupling) 단일-제거 arm.
#   = main − dec = coupled + vanilla = D0 격자점(옛 gvqi3cgq와 같은 위치, 단 현세대:
#     post-fix·advstd·fp32·queue384 → §13.2가 지적한 옛D0 6-인자 오염을 해소하는 정합 baseline).
# 델타 (nocispo 대비 정확히 C1축 3개):
#   async_hpt.rl_old_logprob_source: entry -> rollout   (coupled anchor)
#   +async_hpt.entry_proximal=recent 제거               (entry 전용)
#   algorithm.rollout_correction.rollout_is: token -> null (+threshold 제거; coupled에선 w≡1)
# C2는 vanilla 유지(loss_mode=vanilla, clip_low=0.2) — main과 동일 g-슬롯이라 C1만 격리.
# 산출: nocispo − D0prime = decoupling 한계기여 ; D0prime = 현세대 both-off baseline.
# steps=200 (nocispo/M5와 matched-step 비교). fresh(resume 아님).
# ============================================================================
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_D0prime_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_D0prime_${RUN_TIMESTAMP}"
...
VAL_DUMP_DIR="${VERL_ROOT}/.cache/val_dump/openr1_async_hpt_qwen25_math_1_5b_D0prime_${RUN_TIMESTAMP}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_hpt_D0prime_coupled_vanilla \
