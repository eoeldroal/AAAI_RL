...
# [M3] M2 launcher 대비 정확히 한 학습-로직 델타를 갖는 scratch arm이다:
#   - P0 정직 보상 복원: zero_reward_if_truncated + zero_truncated_rl_advantage
# 즉 M3 - M2 = "보상 정직성" 축의 단일-요인 고립이다 (M2는 mirage 보상으로 신기루
# 래칫이 실측됨: dump 기준 절단-양성 비율 0%→54%→85%, val 26에서 하락 전환).
# SFT 채널은 M2와 동일하게 beta=1.0 + length_inverse(길이중립, 교사궤적당 등가중)로
# 유지해 "정직 보상 + 최강 SFT" 조합을 처음으로 검증한다. 그 외 공통 기반
# (C1 decoupling + C2 CISPO, async stack, trim+carryover 배치)은 M2와 동일하다.
# 관측성 추가: trainer.validation_data_dir — val 생성물이 rollouter actor의
# wandb.run=None 가드에서 침묵 드롭되던 문제를 우회해 JSONL로 확보한다(학습 무관).
# 실행 서약: 500 fit-step 완주. 사전등록 게이트(절단고원·AIME카나리아·ESS<0.6) 외 조기 종료 금지.
#
# [각주 1] 이 스크립트는 Qwen2.5-Math-1.5B용 async RL + HPT의 **M3(honest-reward + strong-SFT) run**이다.
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_M3_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_M3_${RUN_TIMESTAMP}"
...
VAL_DUMP_DIR="${VERL_ROOT}/.cache/val_dump/openr1_async_hpt_qwen25_math_1_5b_M3_${RUN_TIMESTAMP}"
...
#   ours async_hpt.beta=1.0
#     -> tau* SFT row의 terminal pseudo reward를 1.0으로 둔다.
# 따라서 M2는 UPT 1.5B 계수(0.3) 정합 arm이 아니라, score-oriented SFT 강화 arm이다.
#
# [각주 5] M2는 M 대비 score-oriented SFT 강화 arm이다. 이는 원 HPT를 그대로 복제하기
# 위한 변경이 아니라, 현재 fork의 seq-mean-token-sum-norm 아래에서 (1) 긴 tau가 더 크게
# 들리는 길이 편향을 제거하고 (2) SFT 총량을 M(beta=0.3) 대비 키우는 국소 개입이다.
#
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_hpt_M3_p0_leninv_beta1 \
