...
RESUME_FROM_STEP="${RESUME_FROM_STEP:-180}"
RESUME_FROM_PATH="${VERL_ROOT}/checkpoints/async-hpt-openr1/qwen25_math_1_5b_openr1_async_hpt_M5abl_nocispo/global_step_${RESUME_FROM_STEP}"

# ============================================================================
# [M5abl_nocispo_cont = nocispo(=신 main 후보) 이어서 학습] 2026-07-10
# 맥락: nocispo(oki4kv8u)가 폭풍벽 통과+단조상승(40.17@170>LUFFY 39.58)이라, 200 캡을
#   넘겨 더 태운다. baseline(M5abl_nodec)을 갭에 돌린 뒤 이 런처로 재개.
# 델타 (원 nocispo 대비):
#   trainer.resume_mode=disable -> resume_path + resume_from_path=<nocispo>/global_step_${RESUME_FROM_STEP}
#   trainer.total_training_steps 200 -> 300  (이어서 +100)
#   experiment_name -> _cont (M5R 검증 패턴: 원본 nocispo 체크포인트 dir 보존, 신 dir에 누적)
# 손실축(vanilla+clip_low0.2+디커플링유지)은 원 nocispo와 100% 동일 — 순수 continuation.
# 주의: RESUME_FROM_STEP은 저장된 체크포인트여야 함(save_freq=10 → 180/190/200 중 택).
# ============================================================================
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_M5abl_nocispo_cont_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_M5abl_nocispo_cont_${RUN_TIMESTAMP}"
...
VAL_DUMP_DIR="${VERL_ROOT}/.cache/val_dump/openr1_async_hpt_qwen25_math_1_5b_M5abl_nocispo_cont_${RUN_TIMESTAMP}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_hpt_M5abl_nocispo_cont \
