...
# ============================================================================
# [M5abl_nodec = M5 − C1(decoupling)] Ablation_RL.md §13.2 신 격자 2순위 arm (2026-07-09)
# 델타 (M5 대비 정확히 3 + 스텝캡):
#   async_hpt.rl_old_logprob_source: entry -> rollout  (coupled anchor로 복귀)
#   +async_hpt.entry_proximal=recent 제거              (entry 전용 opt-in — coupled에선 설정 금지)
#   algorithm.rollout_correction.rollout_is: token -> null (+threshold 라인 제거; coupled에선 w≡1 inert)
...
# C2(CISPO)는 유지: loss_mode=cispo + clip_low=10.0 + clip_high=0.28 (C_g=1.28 동일값 고정 —
#   이동용 캡이 coupled ρ(staleness 포함)에 걸리는 "캡 충돌"(DR-005 §4.2c)을 그대로 실측하는 arm).
#   validation은 coupled+cispo를 WARN으로 허용(이 arm의 존재가 이유, §3 M−dec).
# 판독(§3 M−dec, §12.3): 융합 clip-frac 상승·C_g 상단 포화, M5−(M5−dec) = decoupling 한계 기여.
# 규율: 폭풍 창 val 판정 제외 · 50-90 창 평균 비교 · 서명 1차(§8).
# ============================================================================
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_M5abl_nodec_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_M5abl_nodec_${RUN_TIMESTAMP}"
...
VAL_DUMP_DIR="${VERL_ROOT}/.cache/val_dump/openr1_async_hpt_qwen25_math_1_5b_M5abl_nodec_${RUN_TIMESTAMP}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_hpt_M5abl_nodec \
