...
RESUME_FROM_STEP="${RESUME_FROM_STEP:-95}"
...
# ============================================================================
# [M5R = M5-resume@95, 무변경 속행] 2026-07-09
#
# 결정 맥락 (docs/Improvement_RL.md §5.12 후속):
#   - M7(4델타: B0+B1'+B2+B3)은 step 100-130에서 만성 진동(재점화 3회, ESS 최저 0.079,
#     score 기준선 미복귀)으로 사전등록 kill-gate 발화 → 중단.
#   - cispo_klcov(B2)는 이번 런에서 학습 이점 미증명: 정점 38.33<38.47(M5), matched-step
#     우위(+0.34@90)는 val 진동 이내, 엔트로피 플로어 방어도 실패(M7 0.10-0.13 < M5 0.14-0.33).
#     → 시간 제약 하에 미증명 카드 재투입 기각. 유일한 실측-앵커 경로 = M5 속행.
#   - M5는 step 96에서 건강 상태로 외부 SIGTERM 종료(M7 기동). 정점 38.47@50 체크포인트는
#     소실됐고 최신 완전 체크포인트 = global_step_95 (val 37.42, entropy 0.19-0.28, score 0.65).
#
# 델타: **없음** (config는 M5와 비트단위 동일; resume 배선 3줄만 추가).
#   clip_grad=80 유지 — M5 폭풍(step 81-84, KL 23.1)은 1스텝 자가회복했고, clip=1.0이던
#   M7이 최심 폭풍을 낸 실측 때문에 "보험" 논리 기각. 깨끗한 속행이 최우선.
#
# 기대값(정직, 40스텝 정체 실측 기반): 밴드 36.6-38.1 진동, ~150스텝(~31회 val) 최선
#   ~38.3-38.6, P(39+)~5%. 목적: (1) 디스크에 38급 체크포인트 재확보(현 보유 최고 38.07@75),
#   (2) M5 250스텝 완주 곡선 확보. 돌파 기대 아님 — 수확 런이다.
#
# 감시/규율:
#   - 정점 즉시 격리: 신규 val 정점 시 해당 global_step_*를 checkpoints/harvest/로 즉시 복사.
#   - 폭풍 시 판정: 폭풍 중 val은 판정 제외(M5 val@80=0.36 전례), 나디르 후 ~10스텝 내
#     ESS>0.8 복귀가 정상(M4 2회·M5 1회 전례). 만성 진동(재점화 2회+)이면 그때 중단.
#   - 실행 전제: M7 프로세스 종료 후 기동(GPU 8장 공유).
# ============================================================================
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_M5R_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_M5R_${RUN_TIMESTAMP}"
...
VAL_DUMP_DIR="${VERL_ROOT}/.cache/val_dump/openr1_async_hpt_qwen25_math_1_5b_M5R_${RUN_TIMESTAMP}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_hpt_M5R_resume95 \
