...
# ============================================================================
# [M″ = Mprime2] M′ 공통 기반 위에서 length_inverse를 scratch로 검증하는 독립 arm.
# M′(Mprime_v2) 대비 델타는 정확히 둘:
#   1) async_hpt.sft_beta_mode=length_inverse — SFT 행 가중을 beta·8192/len(tau)로
#      바꿔 긴-CoT tau의 token-sum 지배를 제거하고 짧게 종결하는 시범에 상대 가중.
#   2) async_hpt.beta=0.6 — 평균 tau 길이 기준 기존 beta=1.0과 총량 파리티를 맞추되,
#      길이 재가중의 과증폭은 피한다.
# 실행은 resume 없이 바닥부터 시작한다. 이어받기 런은 실험 귀속을 흐리고 공정 비교를
# 망가뜨리므로 이 파일에서는 금지한다.
# entropy_coeff=0.0 유지(사유: 필요성 미성립 + truncated-행 entropy 채널 미패치).
# ============================================================================
...
# [각주 0 · Mprime2 = scratch arm] 이 스크립트는 M run(_M.sh) 공통 기반 위에 docs/Improvement_RL.md
# §5의 P0 조치와 M″ 길이 개입(length_inverse + beta=0.6)을 얹은 **독립 scratch run**이다.
# M.sh/Mprime.sh는 기존 앵커로 보존하고, Mprime2는 실험명만 분리해 새 arm으로 기록한다:
...
#   - P0-3: actor.entropy_coeff=0.0 + actor.calculate_entropy=True (entropy bonus 제거, 진단지표 유지)
#   - M″-1: async_hpt.sft_beta_mode=length_inverse
#   - M″-2: async_hpt.beta=0.6
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_Mprime2_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_Mprime2_${RUN_TIMESTAMP}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_hpt_Mprime2_v2 \
