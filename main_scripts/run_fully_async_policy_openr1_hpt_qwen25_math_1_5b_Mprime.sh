...
# [각주 0 · M′ = 기반 v2] 이 스크립트는 M run(_M.sh)에서 docs/Improvement_RL.md §5의 P0 조치를
# 켠 **M′ run(기반 v2, 새 앵커)**이다. M.sh(이미 실행·분석 완료)는 재현성을 위해 보존하고, 델타만
# 여기서 켠다:
...
#   - P0-4: async_hpt.beta=1.0 (SFT 강화; 되돌림 기준: step40까지 clip>0.3 or entropy_mean>2.5면 0.3 회귀)
# 트립와이어(수동 감시): clip_ratio>0.3 지속 / entropy_mean>2.5 / entropy_mean<0.2 / KL(rollout↔train)>1nat / AIME 카나리아.
# 이 변경은 Ablation_RL.md §2 공통 기반을 바꾸므로 M−D0 델타가 오염됨 → 기반 v2로 재앵커(Improvement_RL.md §5.4).
#
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_Mprime_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_Mprime_${RUN_TIMESTAMP}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_hpt_Mprime_v2 \
