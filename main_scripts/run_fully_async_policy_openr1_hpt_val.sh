...
# ============================================================================
# [LUFFY-val] LUFFY 1.5B 공개 체크포인트를 동일 파이프라인으로 validation만 찍는 근사 벤치마크 arm.
# async main 경로는 val_only를 우아하게 지원하지 않으므로, val_before_train=True를 유지한 채
# rollout.total_rollout_steps=0, trainer.total_training_steps=0으로 내려 validation 이후 즉시
# 종료되게 한다. project는 공유하고 experiment만 분리한다.
# ============================================================================
...
MODEL_PATH="${VERL_ROOT}/models/LUFFY-Qwen-Math-1.5B-Zero"
...
ROLLOUT_DUMP_DIR="${VERL_ROOT}/.cache/rollout_dump/openr1_async_hpt_qwen25_math_1_5b_LUFFY_val_${RUN_TIMESTAMP}"
...
TRAIN_DUMP_DIR="${VERL_ROOT}/.cache/train_dump/openr1_async_hpt_qwen25_math_1_5b_LUFFY_val_${RUN_TIMESTAMP}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    trainer.experiment_name=qwen25_math_1_5b_openr1_async_hpt_LUFFY_val \
